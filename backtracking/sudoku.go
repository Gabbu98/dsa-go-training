...
func sudoku(board *[][]int, i *int, perm []int, used []bool, done *bool) {
	...
	for x:=0; x<len(used); x++ {
		...
		if !used[x] && !isPresent(*board, len(*board)-1, len(perm), x ) {
			used[x]=true
			perm = append(perm, x)
...
func isPresent(board [][]int, row int, column int, value int) bool {
	...
	// 3*3
	for i:=0; i<len(board); i++ { // iterating rows
		
		for j:=0; j<len(board[i]); j++ { // iterating columns
			
			// current pos -2 rows & +2 rows

			// current pos -2 columns & +2 columns
			if (i >= acceptableMinusRow && i <= acceptablePlusRow) && (j >= acceptableMinusColumn && j <= acceptablePlusColumn) {
				if board[i][j] == value {
					isPresent = true
					break
				}
